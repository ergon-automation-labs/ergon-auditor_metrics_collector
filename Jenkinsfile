pipeline {
  ...
  environment {
    BOT_NAME = 'metrics_collector_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-auditor_metrics_collector"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/metrics_collector_bot/bin/metrics_collector_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorMetricsCollector.Release.create()'"
          $RELEASE_BIN eval 'BotArmyAuditorMetricsCollector.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorMetricsCollector.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyAuditorMetricsCollector.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_auditor_metrics_collector/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_auditor_metrics_collector/releases/start_erl.data)
        fi
