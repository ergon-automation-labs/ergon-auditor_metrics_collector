# Getting Started with bot_army_auditor_metrics_collector

This guide walks you through setting up Metrics Collector Bot for local development.
...
DATABASE_NAME=bot_army_auditor_metrics_collector_dev
```
...
Metrics Collector Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_auditor_metrics_collector/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_auditor_metrics_collector/skills/example.ex lib/bot_army_auditor_metrics_collector/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyAuditorMetricsCollector.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_auditor_metrics_collector.command.classify"]
...
```elixir
# In lib/bot_army_auditor_metrics_collector/application.ex or similar

defmodule BotArmyAuditorMetricsCollector.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyAuditorMetricsCollector.Skills.Classify,
      BotArmyAuditorMetricsCollector.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_auditor_metrics_collector/skills/my_skill.ex
...
This creates a release named `metrics_collector_bot` that is deployed to `/opt/ergon/releases/metrics_collector_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/metrics_collector_bot/`
